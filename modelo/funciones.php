...
function subirFoto($archivoFoto,$carpeta,$tamanoMaxArchivo = 5000000){
    ...
    $ruta = "";
    $first_nameArchivo = $archivoFoto['name'];
    ...
    if((strpos($tipo, "jpeg") || strpos($tipo, "png")) && $tamano < $tamanoMaxArchivo ){
        $first_nameArchivo = limpiar_caracteres_especiales($first_nameArchivo);

        // reviso si ya existe algun archivo con el mismo first_name en la carpeta.
        if(file_exists($carpeta.$first_nameArchivo)){
            $first_nameCortado = cortarCadenaFinal($first_nameArchivo);
            $numeroAletario = time();
            ...
            $first_nameArchivo = $first_nameCortado."_".$numeroAletario.$extension;
        }
        if(move_uploaded_file($archivoFoto['tmp_name'], $carpeta.$first_nameArchivo)){
            $ruta = $first_nameArchivo;
        }else{
...
// eliminamos los caracteres desde $subcadena hacia la izq, y le sumamos 1 para borrar tambien el @ en este caso
    $first_name = substr ($cadena, 0, ($posicionsubcadena));
    return $first_name;
